...
module.exports = Compiler = ( function() {
    ...
    Compiler.prototype.compile = function( sassFilePath ) {
        ...
        // OLD CODE THAT PUTS IT IN THE SAME FOLDER:
        // var cssFilePath = path.join(path.dirname(sassFilePath),cssFileName);

        console.log("my tries");
        var parentFolder = path.normalize(sassFilePath + "../..");
        var parentFolder = path.dirname(parentFolder);
        var cssFolder = path.join(parentFolder, "/css");
        console.log(cssFolder);
        var cssFilePath = path.join(cssFolder,cssFileName);
        console.log(cssFilePath)
            // console.log( path.join(path.dirname(path.normalize(sassFilePath + "../..")), "/css") )
            // console.log( path.join(path.join(path.dirname(path.normalize(sassFilePath + "../..")), "/css"),cssFileName) )
